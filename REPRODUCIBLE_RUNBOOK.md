...
If raw data have been restored locally, the manuscript-used output route is:
...
The repository also retains older diagnostic scripts whose names begin with `a03`, `a04`, `a05`, `a13`, `a19`, and `a20` only if present in a local working copy. They are not the final manuscript-output route. Manuscript-used figures are generated by `fig_build.py`, `a18_mechanism_v2.py`, `a21_fig.py`, and `a27_column_fig.py`.
