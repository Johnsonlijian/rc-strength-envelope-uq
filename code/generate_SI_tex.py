"""Emit a compilable LaTeX Supplementary Information from saved public outputs."""
...
MS = Path("../outputs")
TITLE = (
    "Machine-learned reinforced-concrete strength predictions fail beyond the "
...
\noindent All tables are emitted directly from saved public analysis outputs by \texttt{{code/generate\_SI\_tex.py}}. Full raw-data regeneration requires restoring the third-party source records listed in \texttt{{DATASETS\_AND\_LINKS.csv}}.
"""
]
...
L.append(
    table(
        ...
        "Real RC member test databases and analysed subsets.",
        "tab:s1",
        colfmt="llrrr",
    )
)
...
L.append(table(pd.DataFrame([[k, v["mean"], v["cov"]] for k, v in mu.items()], columns=["model", "mean M", "COV"]), "Mechanical-model validation on the canonical steel benchmark.", "tab:s2"))
L.append(table(pd.read_csv(PROC / "steel_uq_matrix.csv"), "ML interval coverage on the canonical steel slender-beam benchmark.", "tab:s3", fmt=3))
...
L.append(table(frp_deep, "External-database UQ matrix including the saved SFRC replication artifact.", "tab:s4", fmt=3, colfmt="lllrr"))
L.append(table(pd.read_csv(PROC / "steel_gpu_uq.csv"), "GPU-trained deep-UQ coverage; split-level metadata are saved in \\texttt{steel\\_gpu\\_uq\\_raw.csv}.", "tab:s5", fmt=3))
...
L.append(
    table(
        ...
        "Large-size exponent $m$ (size effect): real data, FE, mechanical theories, and ML extrapolation artefact.",
        "tab:s6",
    )
)
...
L.append(table(rel[[c for c in rel_cols if c in rel.columns]], "Realised reliability index $\\beta$ vs. member size.", "tab:s7", fmt=3))

ce = pd.read_csv(PROC / "column_extrap.csv")
ce.columns = ["axis", "model", "interp", "extrap", "bias", "cov", "unsafe"]
ce["% unsafe"] = (100 * ce["unsafe"]).round(0)
L.append(table(ce[["axis", "model", "interp", "extrap", "bias", "cov", "% unsafe"]], "RC columns: ML interval coverage under interpolation vs. extrapolation.", "tab:s8", fmt=3))

col_mech = maybe_column_mech_summary()
if col_mech is not None:
    L.append(table(col_mech, "Column mechanical model aggregate by axial-load bin.", "tab:s9", colfmt="lrrrr"))
else:
    L.append(r"\paragraph{Column mechanical aggregate boundary.} Restore the third-party column records and rerun \texttt{a26_column_mech_validate.py}, or use the archived \texttt{column_mech_by_axial_bin.csv} if present.")

if (PROC / "fe_mesh_objectivity.json").exists():
    mesh = json.load(open(PROC / "fe_mesh_objectivity.json", encoding="utf-8"))
    mesh_df = pd.DataFrame(mesh["summary"]).rename(columns={"d_mm": "d (mm)", "min_v_nom_mpa": "min v_nom", "max_v_nom_mpa": "max v_nom", "relative_range": "rel. range"})
    L.append(table(mesh_df, "Mesh-objectivity check for the crack-band FE model under $2\\times$ refinement.", "tab:s10", fmt=3))

protocol = pd.DataFrame(
    [
        ["Steel UQ matrix", "a16_steel_extrapolation.py", "0.70, 0.75, 0.80", "0, 1, 2", "steel_uq_matrix.csv"],
